...
def getIDs(model):
    ...
    for i in ids:
        print (str(count) +': ' + (i))
        count +=1
    print ('What is output? [Pick one]')
    output = input()
    print ('What are your variables? (Put \',\' in between index) [Pick up to 6]')
    variables = input()
    origIds = ids
...
def createParaMat(paraMatrix,simList,selIdsCount):
    ...
    count = -1
    paramRangeNum = 10  #turn into logspace
...
#save variables in order to examine later if desired. Timestamped.
time = time.strftime('%Y%m%d%H%M%S')
np.save('./data/totalData_'+time+'.npy', totalData)
np.save('./data/dataIds_'+time+'.npy', dataIds)
np.save('./data/indDataIds_'+time+'.npy', indDataIds)
#load them up
#totalData = np.load('./data/totalData.npy')
#dataIds = np.load('./data/dataIds.npy')
#indDataIds = np.load('./data/indDataIds.npy')
...
#each element in the 2D will have a set of size Variable results according to the current parameter in that simulation.
#Variable [_,:,:] = idsCount-paramCount, Sensitivity[:,_,:], Parameter [:,:,_] = paramCount.

#sens = np.full((idsCount-paramCount,len(totalData.T)-1,paramCount),np.nan,dtype=np.float64)
## set ground truth
#V_t = totalData[:-paramCount,0]
#sim = np.empty([idsCount,1],dtype=np.float64)
#for idx,sim in enumerate(totalData.T):
#    for x in np.arange(0,paramCount):
#        P_t = totalData[-x,0]
#        P = sim[-x]
#        V = sim[:-paramCount]
#        sens[:,idx-1,x] = sensAnalysis(V,P,V_t,P_t)

#%% pickle saving of figures is not working :(
#import pickle as pl
#pl.dump(fig,open('fig.pickle','wb'))
#fig_handle = pl.load(open('fig.pickle','rb'))
#fig_handle.show()